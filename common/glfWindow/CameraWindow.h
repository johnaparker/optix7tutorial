#include "glfWindow/GLFCameraWindow.h"
#include <GL/gl.h>

namespace optix7tutorial {
  struct CameraWindow : public GLFCameraWindow
  {
    CameraWindow(const std::string &title,
                 const Camera &camera,
                 const float worldScale,
                 const int2 size=make_int2(1200,800))
      : GLFCameraWindow(title,camera.from,camera.at,camera.up,worldScale, size) {}

    virtual void render() override {
      if (cameraFrame.modified) {
        scene.setCamera(Camera{ cameraFrame.get_from(),
                                 cameraFrame.get_at(),
                                 cameraFrame.get_up() });
